...
///firebase_define//////
#define FIREBASE_HOST "https://pty7xkusrc-55d6b.firebaseio.com/" //Do not include https:// in FIREBASE_HOST
#define FIREBASE_AUTH "Gl5NavdnT7pZSQuLGwNm5VsDpllBqjnwJ1HGaas4"
FirebaseJson json;

///MQTT_define/////
#define MQTT_SERVER "35.192.38.248"
#define MQTT_PORT 1883
#define MQTT_USER "pty7xkusrc"
#define MQTT_PASSWORD "{pty7}xKUSRC"
#define MQTT_TOPIC "fromMachine"
...
void display_phonenumbertofirebase(){
        if(show == 0){
          ...
          while(true){
          char keypressed = myKeypad.getKey();  
            if (keypressed != NO_KEY && phonenum.length() < 3){
                    phonenum += keypressed;
                    tft.setFont(Terminal11x16);        
            ...
            else if(keypressed == '#' && phonenum.length() == 3){
            tft.clear();
            for(int i =0;i<=3;i++){
...
void display_getuser()
{
  ...
  Serial.println(phonenum); 
  String path = "/student/"+phonenum;
  FirebaseData firebaseData;
  Firebase.begin(FIREBASE_HOST, FIREBASE_AUTH); 
  Firebase.reconnectWiFi(true);
  Firebase.getString(firebaseData, path+"/eng_first_name");
  ...
  Serial.println(uage);
  Firebase.getString(firebaseData, path+"/sex");
  String usex = firebaseData.stringData();
  Serial.println(usex);
  Firebase.getString(firebaseData, path+"/last_weight");
  String l_weight = firebaseData.stringData();
  Serial.println(l_weight);
  Firebase.getString(firebaseData, path+"/last_height");
  String l_height = firebaseData.stringData();
  Serial.println(l_height);
  Firebase.getString(firebaseData, path+"/last_bmi");
  String l_bmi = firebaseData.stringData();
 ...
 if(uname != "" && uage != "")
 {
  ...
  tft.drawText(75, 20, "Name ");
  tft.drawText(75, 40, "Age ");
  tft.drawText(75, 60, "Sex ");
  tft.drawText(20, 90, "Weight ");
  tft.drawText(20, 120, "Height ");
  tft.drawText(20, 150, "BMI ");
  tft.drawText(125, 20, uname);
  tft.drawText(125, 40, uage);
  tft.drawText(125, 60, usex);
...
void put_toMQTT(String sw , int sh , float sb)
{
  String clientId = "ESP32Client-";
  String payload_toMQTT = "{\"weight\":\"";
  payload_toMQTT +=sw;
  payload_toMQTT +="\",\"height\":\"";
  payload_toMQTT +=convertFloatToString(sh);
  payload_toMQTT +="\",\"bmi\":\"";
  payload_toMQTT +=convertFloatToString(sb);
  payload_toMQTT +="\",\"studentID\":\"";
  payload_toMQTT +=phonenum;
  payload_toMQTT +="\",\"type\":\"student";
  payload_toMQTT +="\"}";
  //+","+"\"height\""+":"+"\""+convertFloatToString(sh)+"\""+","+"\"bmi\""+":"+"\""convertFloatToString(sb)+"\""+","+"\"phonenumber\""+":"+"\""+phonenum+"\""+"}";
  ...
  client.publish(MQTT_TOPIC, payload_toMQTT.c_str());
  Serial.print("SentMQTT!!!");   
}
...
void passtobmiShow(){
  ...
  if (keypressed != NO_KEY && hight.length() < 3){
    ...
    if(renew == 99 && hight.length() == 3){
      ...
      if(sw_to == 1)
      {
       put_toMQTT(compareweight[0],hight.toInt(),sum);
      }      
      for(int i = 0;i<30;i++){
        Serial.print("i : ");
        Serial.println(i);
...
void setup() {
    ...
    tft.begin(vspi);
    tft.setOrientation(3);   
    scale.set_scale(12754);  // Start scale
    scale.tare();
